...
router.post('/add', authenticateToken, isAdmin, (req, res) => {
  ...
  if (!user_id || !date || !time || !type || !justification) {
    return res.status(400).json({ error: 'Todos os campos s�o obrigat�rios, incluindo justificativa' });
  }
  ...
  db.get(
    ...
    (err, existing) => {
      ...
      if (existing) {
        console.log('� Registro j� existe para este tipo nesta data');
        return res.status(400).json({ error: 'J� existe uma batida deste tipo para esta data. Use a fun��o de editar.' });
      }
...
router.put('/edit/:id', authenticateToken, isAdmin, (req, res) => {
  ...
  if (!time || !justification) {
    return res.status(400).json({ error: 'Hor�rio e justificativa s�o obrigat�rios' });
  }
...
router.delete('/delete/:id', authenticateToken, isAdmin, (req, res) => {
  ...
  if (!justification) {
    return res.status(400).json({ error: 'Justificativa � obrigat�ria para exclus�o' });
  }
  ...
  db.get('SELECT * FROM time_records WHERE id = ?', [id], (err, record) => {
    ...
    db.run('DELETE FROM time_records WHERE id = ?', [id], function(err) {
      ...
      console.log(' Batida exclu�da. Changes:', this.changes);
      console.log('=� Justificativa:', justification);
      res.json({ message: 'Batida exclu�da com sucesso' });
    });
  });
});
...
router.get('/records/:userId/:date', authenticateToken, isAdmin, (req, res) => {
  ...
  console.log('=� Par�metros:', { userId, date });
