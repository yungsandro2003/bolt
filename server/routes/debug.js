...
router.post('/seed-scenarios', authenticateToken, isAdmin, (req, res) => {
  ...
  db.run(
    ...
    function(err) {
      if (err) {
        console.error('L Erro ao limpar registros antigos:', err);
      ...
      scenarios.forEach((record, index) => {
        db.run(
          ...
          function(err) {
            if (err) {
              console.error(`L Erro ao inserir registro ${index}:`, err);
            } else {
              inserted++;
            ...
            if (index === scenarios.length - 1) {
              ...
              console.log(`   Dia 2 (${day2}): Esquecimento - Apenas entrada, sem sa�da`);
                ...
                scenarios: [
                  {
                    ...
                    details: 'Entrada e sa�da batendo com o turno'
                  },
                  {
                    date: day2,
                    description: 'Esquecimento',
                    details: 'Entrada existe, sem sa�da'
                  },
                  {
                    date: day3,
                    description: 'Atraso',
