...
export function ManualAdjustments() {
  ...
  const loadEmployees = async () => {
    try {
      ...
      showMessage('error', 'Erro ao carregar lista de funcion�rios');
    }
  };
  ...
  const handleSubmit = async () => {
    if (!formData.justification.trim()) {
      showMessage('error', 'A justificativa � obrigat�ria');
      return;
    }
    ...
    try {
      if (modal.mode === 'add') {
        ...
        showMessage('success', 'Batida exclu�da com sucesso');
      }

      closeModal();
      loadRecords();
    } catch (error: any) {
      showMessage('error', error.response?.data?.error || 'Erro ao processar opera��o');
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p style={{ color: '#E0E0E0', opacity: 0.7 }} className="text-sm">
          Adicionar, editar ou excluir batidas de ponto com justificativa obrigat�ria
        </p>
      </div>
        ...
        >
          ...
          >
            ...
            <div className="mb-6">
              ...
              />
              <p className="text-xs mt-1" style={{ color: '#6B7280' }}>
                A justificativa � obrigat�ria e ficar� registrada no sistema
              </p>
            </div>
